...
async function run() {
  ...
  if (mode === 'ci') {
    ...
    const reportPath = path.join(process.cwd(), 'verification/chapter-02-verification.json');
    if (!fs.existsSync(reportPath)) {
      console.error("CI Mode Fail: Saved audit report not found at verification/chapter-02-verification.json");
      process.exit(1);
    }
    ...
    if (savedReport.originalQuestionsCount !== 121) {
      errors.push(`originalQuestionsCount is not 121: got ${savedReport.originalQuestionsCount}`);
    }
    if (savedReport.enrichmentQuestionsCount !== 50) {
      errors.push(`enrichmentQuestionsCount is not 50: got ${savedReport.enrichmentQuestionsCount}`);
    }
    if (savedReport.totalQuestionsCount !== 171) {
      errors.push(`totalQuestionsCount is not 171: got ${savedReport.totalQuestionsCount}`);
    }
    ...
    if (savedReport.testStatus !== 'PASS') {
      errors.push(`testStatus is not PASS: got ${savedReport.testStatus}`);
    }

    // Recalculate critical file hashes and compare them
    const filesToVerify = [
  ...
  // Recalculate critical file hashes and store them
  const filesToVerify = [
    'assets/js/questions.js',
    'assets/js/app.js',
    'assets/css/style.css',
